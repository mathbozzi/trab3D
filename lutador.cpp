...
Vector3 Lutador::posicaoOlho()
{
	...
	eyePos.x += head->transform.position.x * sin(bodyRotation * (M_PI/180));
	eyePos.x += head->transform.position.y * sin(bodyRotation * (M_PI/180));
	eyePos.y += head->transform.position.y * -cos(bodyRotation * (M_PI/180));
	eyePos.y += head->transform.position.x * -cos(bodyRotation * (M_PI/180));
    eyePos.z += head->transform.position.z + head->radius;

    return eyePos;
}

Vector3 Lutador::posicaoLuva()
{
    GLfloat bodyRotation = this->transform.rotation.z + 90;
